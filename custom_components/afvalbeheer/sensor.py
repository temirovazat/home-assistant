...
class WasteCollectionRepository(object):
    ...
    def get_upcoming_by_type(self, waste_type):
        ...
        return list(
            filter(
                lambda x: x.date.date() >= today.date() and x.waste_type == waste_type,
                self.get_sorted(),
            )
        )
    ...
    def get_by_date(self, date, waste_types=None):
        if waste_types:
            return list(
                filter(
                    lambda x: x.date.date() == date.date()
                    and x.waste_type in waste_types,
                    self.get_sorted(),
                )
            )
        else:
            return list(
                filter(lambda x: x.date.date() == date.date(), self.get_sorted())
            )
...
class WasteData(object):
    ...
    def __select_collector(self):
        if self.waste_collector in XIMMIO_COLLECTOR_IDS.keys():
            self.collector = XimmioCollector(
                self.hass,
                self.waste_collector,
                self.postcode,
                self.street_number,
                self.suffix,
                self.address_id,
            ...
            self.waste_collector in ["mijnafvalwijzer", "afvalstoffendienstkalender"]
            or self.waste_collector == "rova"
        ):
            self.collector = AfvalwijzerCollector(
                self.hass,
                self.waste_collector,
                self.postcode,
                self.street_number,
            ...
            self.collector = AfvalAlertCollector(
                self.hass,
                self.waste_collector,
                self.postcode,
                self.street_number,
            ...
            self.collector = DeAfvalAppCollector(
                self.hass,
                self.waste_collector,
                self.postcode,
                self.street_number,
            ...
            self.collector = CirculusCollector(
                self.hass,
                self.waste_collector,
                self.postcode,
                self.street_number,
            ...
            self.collector = LimburgNetCollector(
                self.hass,
                self.waste_collector,
                self.city_name,
                self.postcode,
    ...
    async def async_update(self, *_):
        ...
        if self.print_waste_type:
            persistent_notification.create(
                ...
                + ", ".join(self.collector.collections.get_available_waste_types()),
                "Afvalwijzer",
                NOTIFICATION_ID,
            )
...
class AfvalAlertCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(AfvalAlertCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
...
class AfvalwijzerCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(AfvalwijzerCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
...
class CirculusCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(CirculusCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
    ...
    def __get_data(self):
        ...
        if session_cookie:
            ...
            r = requests.post(
                "{}/register/zipcode.json".format(self.main_url),
                data=data,
                cookies=cookies,
            )
            ...
            if self.suffix != "" and json_response_data["flashMessage"] != "":
                ...
                for address in json_response_data["customData"]["addresses"]:
                    if (
                        re.search(
                            " " + self.street_number + " " + self.suffix.lower(),
                            address["address"],
                        )
                        != None
                    ):
        ...
        if logged_in_cookies:
            ...
            response = requests.get(
                "{}/afvalkalender.json?from={}&till={}".format(
                    self.main_url, startDate, endDate
                ),
                headers=headers,
    ...
    async def update(self):
        ...
        try:
            ...
            if (
                not "customData" in response
                or not response["customData"]["response"]["garbage"]
            ):
                _LOGGER.error("No Waste data found!")
                return
            ...
            for item in response["customData"]["response"]["garbage"]:
                for date in item["dates"]:
                    ...
                    collection = WasteCollection.create(
                        date=datetime.strptime(date, "%Y-%m-%d"), waste_type=waste_type
                    )
                    self.collections.add(collection)
...
class DeAfvalAppCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(DeAfvalAppCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
...
class LimburgNetCollector(WasteCollector):
    ...
    def __init__(
        ...
        super(LimburgNetCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
        self.city_name = city_name
    ...
    def __fetch_address(self):
        response = requests.get(
            "{}/afval-kalender/gemeenten/search?query={}".format(
                self.main_url, self.city_name
            )
        ).json()
        ...
        response = requests.get(
            "{}/afval-kalender/gemeente/{}/straten/search?query={}".format(
                self.main_url, self.city_id, self.street_name
            )
        ).json()
    ...
    async def update(self):
        ...
        try:
            ...
            for item in response:
                ...
                collection = WasteCollection.create(
                    date=datetime.strptime(item["date"], "%Y-%m-%dT%H:%M:%S%z").replace(
                        tzinfo=None
                    ),
                    waste_type=waste_type,
                )
...
class OmrinCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(OmrinCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
    ...
    def __get_data(self):
        ...
        response = requests.post(
            "{}/FetchAccount/".format(self.main_url) + self.appId,
...
class OpzetCollector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(OpzetCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
    ...
    def __get_data(self):
        get_url = "{}/rest/adressen/{}/afvalstromen".format(self.main_url, self.bag_id)
...
class RD4Collector(WasteCollector):
    ...
    def __init__(self, hass, waste_collector, postcode, street_number, suffix):
        super(RD4Collector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
...
class RecycleApp(WasteCollector):
    ...
    def __init__(
        self, hass, waste_collector, postcode, street_name, street_number, suffix
    ):
        super(RecycleApp, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
        self.street_name = street_name
    ...
    def __get_access_token(self):
        response = requests.get(
            "{}access-token".format(self.main_url), headers=self.__get_headers()
        )
    ...
    def __get_location_ids(self):
        response = requests.get(
            "{}zipcodes?q={}".format(self.main_url, self.postcode),
        ...
        response = requests.get(
            "{}streets?q={}&zipcodes={}".format(
                self.main_url, self.street_name, self.postcode_id
            ),
            headers=self.__get_headers(),
        )
        if not response.status_code == 200:
    ...
    def __get_data(self):
        ...
        response = requests.get(
            "{}collections?zipcodeId={}&streetId={}&houseNumber={}&fromDate={}&untilDate={}&size=100".format(
                self.main_url,
                self.postcode_id,
                self.street_id,
                self.street_number,
    ...
    async def update(self):
        ...
        try:
            ...
            for item in response["items"]:
                ...
                if (
                    not item["fraction"]
                    or not "name" in item["fraction"]
                    or not "nl" in item["fraction"]["name"]
                ):
                    continue
...
class XimmioCollector(WasteCollector):
    ...
    def __init__(
        ...
        super(XimmioCollector, self).__init__(
            hass, waste_collector, postcode, street_number, suffix
        )
    ...
    def __fetch_address(self):
        ...
        response = requests.post(
            "{}/api/FetchAdress".format(self.main_url), data=data
        ).json()
    ...
    def __get_data(self):
        ...
        response = requests.post("{}/api/GetCalendar".format(self.main_url), data=data)
        return response
...
class WasteTypeSensor(Entity):
    def __init__(
        ...
        self._attr_unique_id = _format_sensor(
            name, name_prefix, waste_collector, self.waste_type
        )
    ...
    def __set_state(self, collection):
        ...
        if self.date_object:
            ...
            if self.day_of_week_only:
                ...
                self._state = collection.date.strftime(
                    self._tomorrow + ", " + date_format
                )
        elif date_diff == 0:
            if self.day_of_week_only:
...
class WasteDateSensor(Entity):
    ...
    @property
    def extra_state_attributes(self):
        return {ATTR_HIDDEN: self._hidden}

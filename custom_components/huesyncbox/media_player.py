...
class HueSyncBoxMediaPlayerEntity(MediaPlayerEntity):
    ...
    async def async_set_sync_state(self, sync_state):
        ...
        try:
            ...
            if "13: Invalid Key" in ex.args[0]:
                ...
                LOGGER.warning(
                    "This error most likely occured because the service call resulted \
                    in an empty message to the syncbox Make sure that the selected options \
                    would result in an action on the syncbox \
                    (e.g. requesting only `sync_toggle:false` would cause such an error)."
                )
            raise
    ...
    async def async_set_bridge(self, bridge_id, username, clientkey):
        ...
        and the bridge_connection_state is `connected`, `invalidgroup` or `streaming`,
        other status means it is connecting.

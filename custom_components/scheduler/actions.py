        ...
        for action in actions:
            ...
            if entity not in self._queues:
                self._queues[entity] = ActionQueue(
                    self.hass, self.id, conditions, condition_type
                )
...
class ActionQueue:
    def __init__(
        self, hass: HomeAssistant, id: str, conditions: list, condition_type: str
    ):
        ...
        for condition in conditions:
            if (
                ...
                and condition[ATTR_ENTITY_ID] not in self._entities
            ):
    ...
    def add_action(self, action: dict):
        ...
        if (
            ...
            and action[ATTR_ENTITY_ID] not in self._entities
        ):
    ...
    async def async_start(self):
        ...
        if self.hass.state != CoreState.running:
            self._startup_callback = async_dispatcher_connect(
    ...
    async def async_process_queue(self):
        ...
        while i < len(self._queue):
            ...
            if not service_is_available(self.hass, action[CONF_SERVICE]):
                i += 1
                _LOGGER.debug(
                    "[{}]: service {} is unavailable, action is postponed".format(
                ...
                i += 1
                _LOGGER.debug(
                    "[{}]: {} is unavailable, action {} is postponed".format(
                        self.id, ", ".join(unavailable_entities), action[CONF_SERVICE]
                    )
                )
            else:
    ...
    async def async_execute_action(self, service_call: dict):
        ...
        if not result:
            _LOGGER.debug(
                "[{}]: conditions have failed, skipping execution of action {}".format(
                    self.id,
                    service_call[CONF_SERVICE],
                )
            )
        ...
        if ATTR_ENTITY_ID in service_call:
            ...
            _LOGGER.debug(
                "[{}]: Executing service {}".format(self.id, service_call[CONF_SERVICE])
            )
    ...
    def start_timer(self, delay: int):
        ...
        self._timer = async_call_later(self.hass, delay, async_timer_finished)
        _LOGGER.debug(
            "[{}]: Postponing next action for {} seconds".format(self.id, delay)
        )

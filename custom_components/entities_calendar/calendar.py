...
CALENDAR_ENTITY_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_START_TIME, default={}): vol.Schema(
            {
                ...
                vol.Exclusive(
                    CONF_TIMESTAMP_IN_STATE, CONF_START_TIME, msg=MSG_TIMESTAMP_CONFLICT
                ): cv.boolean,
            }
        ),
        vol.Optional(CONF_END_TIME, default={}): vol.Schema(
            {
                vol.Exclusive(
                    CONF_TIMESTAMP_ATTRIBUTE, CONF_END_TIME, msg=MSG_TIMESTAMP_CONFLICT
                ): cv.string,
                vol.Exclusive(
                    CONF_TIMESTAMP_IN_STATE, CONF_END_TIME, msg=MSG_TIMESTAMP_CONFLICT
                ): cv.boolean,
            }
        ),
...
def _get_date(options, state_object):
    ...
    if timestamp_in_state is None:
        timestamp_in_state = (
            ...
            else state_object.attributes.get("device_class") == "timestamp"
        )

    if timestamp_in_state:
        if (
            state_object.state == STATE_UNKNOWN
            or state_object.state == STATE_UNAVAILABLE
        ):
            return None
...
class EntitiesCalendarData:
        ...
        for entity in self._entities:
            ...
            if start_date < start < end_date:
                ...
                event = {
                    "uid": entity,
                    "summary": entity.get(
                        CONF_NAME, state_object.attributes.get("friendly_name")
                    ),
    ...
    def update(self):
        ...
        events.sort(
            key=lambda x: x["start"]["date"] if x["allDay"] else x["start"]["dateTime"]
        )
